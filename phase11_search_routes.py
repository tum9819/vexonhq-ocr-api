...
- source: pos_sale, rider_income_grab, rider_income_lineman, ar_payment, manual, ap_payment, bank_statement
- label: รายละเอียด/ชื่อรายการ (text)
- counterparty: ชื่อคู่ค้า/supplier (text)
- amount: จำนวนเงิน (numeric)
- category_code: รหัสหมวดหมู่ค่าใช้จ่าย
...
ตัวอย่าง:
- "หาบิล Makro เดือนเมษา" → {"date_from":"2026-04-01","date_to":"2026-04-30","direction":"expense","keyword":"Makro","amount_min":null,"amount_max":null,"source":null,"category_code":null}
...
def _build_and_run_query(f: SearchFilter, limit: int) -> list[dict]:
    ...
    if f.keyword:
        conditions.append(
            "(d.label ILIKE %s OR d.counterparty ILIKE %s OR d.source ILIKE %s)"
        )
        kw = f"%{f.keyword}%"
...
def get_suggestions():
    """ตัวอย่าง query สำหรับ UI placeholder / hint."""
    return {
        "suggestions": [
            "หาบิล Makro เดือนเมษา",
            "รายรับจาก Grab ทั้งหมด",
