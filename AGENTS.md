...
**74. Export Center Phase A readiness is single-branch, read-only evidence; Audit schema v2 is additive.** (2026-07-29; shipped 2026-08-01.) `GET /export/readiness` accepts only `branch_code=thawi_watthana`. `IMAGE_URLS_PRESENT` counts only a stored attachment row whose `file_url` is empty, or a branch-linked slip with no `raw_image_url`; a bill that was never uploaded belongs to `INVOICE_ATTACHMENT_EVIDENCE`/`CREDIT_CARD_INVOICE_EVIDENCE` instead (owner-approved deviation from the Phase A plan table, 2026-08-01); TUM confirmed legacy `vendor_bills.branch_code IS NULL` belongs to that branch, so scope NULL bills with the fixed authority value only—never a caller-provided fallback. Statement `MIN/MAX(txn_date)` and `pos_imports.period_start/period_end` are observed transaction dates, not the PDF-declared period, and cannot prove full-month coverage; `DAYBOOK_RECONCILES` also remains `preview_only` until an independent comparator exists. Acquire all readiness SELECTs plus Monthly Close checks in one read-only `REPEATABLE READ` snapshot and fingerprint every fact-affecting row field. Audit schema v2 adds complete `evidence_vouchers` while legacy `vouchers`/`voucher_count`/`voucher_total` remain cash-basis-only. Prove Statement membership by joining `bank_statement_entries`; manual/AP/non-Statement rows are `Other` and never require a transfer slip. Waive a Statement row's missing-slip blocker only when its linked bill—regardless of bill month—has `review_status='confirmed'` **and** (`payment_type='credit_card'` **or** `payment_status='credit_card'`); rejected, pending, NULL, or non-card bills do not qualify.

**73. Receipt counts and invoice discounts must use their canonical sources consistently.** (Regression 2026-07-16; external review approved 2026-07-17; shipped 2026-07-17.) In `/pnl/monthly`, `v_daybook` contains one `pos_sale` row per operating day, so `COUNT(DISTINCT ref_id)` counts days, not receipts; sum `public.pos_sales_daily.bill_count` by branch/year/month instead, while leaving cash-basis money totals on the existing P&L source. For invoice completeness, every path that calls `_items_tie_state` (confirm, monthly-SKU, Re-OCR preview, and Re-OCR apply) must pass the same whole-bill discount amount/percent parsed from `ocr_json.discount`; otherwise a valid bill discount over the ordinary tie band is falsely reported incomplete. The legacy monthly-SKU `paid_*` response keys are deploy-compatibility names only: values are confirmed bill totals grouped by `bill_date` and can include unpaid AP, so UI/docs must not call them paid or slip-matched amounts.
...
**62. POS `bill_detail` import timeout was CONCURRENCY (overlapping re-uploads), NOT slow SQL — fixed with a duplicate short-circuit + per-scope advisory lock + bulk RETURNING write; the per-line-item `SELECT` loop was the latent amplifier.** (Session 76, 2026-06-26, `pos_import.py`, commit `68858ad`, rollback `backup-pre-pos-import-fix-2026-06-26`) SYMPTOM: the `รายงานยอดขายแยกตามรายละเอียดบิล.xlsx` upload showed **`ล้มเหลว` / `canceling statement due to statement timeout` / `while inserting index tuple (…) in relation "pos_bills"`** on the web; other files showed duplicate. ROOT CAUSE (two coupled causes — confirmed from `pos_imports` timing, NOT guessed; the first single-cause "lock contention" narrative was CORRECTED by the workflow red-team): **(1) DOMINANT** — `uq_pos_imports_hash` is a **PARTIAL** unique index (`UNIQUE(file_hash) WHERE status='success'`), so a re-upload's `'parsing'` INSERT never conflicts early; the OLD code re-ran the **entire heavy write** and only hit the dup at the final `UPDATE status='success'`. That write contained an **O(N) per-line-item `SELECT id FROM pos_bills` loop** (~4178 round-trips/month) — a lone re-import (`5ab280fe`) took **626 s with ZERO concurrent bill_detail import** (≈150 ms/round-trip under pooler latency), while a warm single import does the same work in **~1.2 s**. **(2) SECONDARY (the actual timeout)** — genuine lock contention on the `uq_pos_bills` unique index, but ONLY on the last attempts that overlapped a still-running slow re-import, tripping the 2-min `statement_timeout`. **NOT** slow business SQL, missing index (`uq_pos_bills(branch_code,receipt_code,sales_date)` is an exact match), bloat, or RLS (backend connects as `postgres`, RLS bypassed). **NO DATA LOSS** — June 1–25 (623 bills, ฿224,569) was already imported by the 23:47 success; the `ล้มเหลว` rows were failed RE-UPLOADS of already-imported data. FIX (both `/pos/import` background worker AND legacy `/import_sync`): **(A)** before any work, `SELECT … WHERE file_hash=%s AND status='success'` → return `already_imported` (no row, no index touch) — kills the wasted 10-min re-runs + the failed-row noise; (B) **session** `pg_try_advisory_lock(0x504F53, crc32(branch:rtype))` serialises same-scope imports (`~5s` bounded retry, `conn.rollback()` between tries to avoid idle-in-transaction, released in `finally`/on disconnect) + `SET lock_timeout='10s'` backstop; **(C)** `_bulk_upsert_pos_bills` = ONE `INSERT … ON CONFLICT … DO UPDATE … RETURNING` via `psycopg2.extras.execute_values(fetch=True)` builds `{(branch,receipt,date)→id}` in memory, and `_bulk_insert_sales_items` resolves `bill_id` from that map with an **order-independent batch-SELECT fallback** (no per-item loop). **INVARIANT (do NOT break):** the RETURNING columns are the ON CONFLICT key and MUST NOT be in `update_cols`, so RETURNING echoes the incoming values → the map key equals the parser's `_bill_key` for both inserted AND updated rows. VERIFIED: **15/15 real-DB integration test** (advisory-lock serialises cross-connection; RETURNING id-map equality; idempotent re-import = NO duplicate items; fallback), all writes ROLLED BACK; `EXPLAIN` shows `Conflict Arbiter Indexes: uq_pos_bills`; Codex ×2 + workflow red-team/2 lenses; Codex re-test = batch end-to-end SUCCESS. DB cleanup (one-time, `apply_migration`-free, via Supabase SQL): **72 noise rows → `status='cancelled'`** (41 stuck `parsing` + 29 duplicate + 2 timeout; all had 0 `pos_bills` referencing them), **4 genuine `failed` rows PRESERVED** (a 2026-05-31 audit backfill + three real `uuid = text` cast failures). DEFERRED (lean-bar): `batch_import_local.py` (local CLI, not the failing path, not concurrent) still has the old per-item loop; multi-branch (`file_hash`/lock are global — correct vs the global partial index today, revisit index+check together if multi-branch lands). See FE AGENTS #20 (the upload-history badge fix for the new `cancelled` status).

**63. Makro multi-page OCR: Payment `TOTAL` is VAT-inclusive before discount, NOT JSON `subtotal` when the VAT breakdown exists.** (2026-07-01) For Makro invoices the last page can show an empty item table plus VAT summary and payment box. Use the VAT breakdown `รวม` row as the app's accounting fields: `subtotal = ราคาสินค้า`, `vat = ภาษี`, `amount = รวม`, so `subtotal + vat = amount`. Payment `TOTAL` (e.g. 1,125.25) is VAT-inclusive before bill discount and is only a cross-check; using it as `subtotal` creates false `VAT_MISMATCH`. Also never insert summary labels as line items: GPT may emit the VAT summary row as `product_name="จำนวนชิ้น"`, `amount=684.25`; `_is_real_item` must drop labels like `จำนวนชิ้น`, `ราคาสินค้า`, `ภาษีมูลค่าเพิ่ม`, `รวม`, `VAT/TOTAL/AMOUNT`. On multi-page merge, keep page-1 real items in `ocr_json.items` but merge summary-page totals/discount into `ocr_json` so `_revalidate_bill` can explain `items_total - discount = amount` without false warnings. Guarded by `tests/test_merge_totals.py`, `tests/test_invoice_validation.py`, and `tests/test_ocr_structured_wiring.py`.

**64. OpenAI API key rotation incident: LINE Slip OCR 401 `Incorrect API key provided` usually means the backend is using the wrong/invalid OpenAI key, NOT Claude credit and NOT normal quota exhaustion.** (2026-07-06, ops-only, no code change) SYMPTOM: sending a K+ slip image to LINE replied `OCR failed: Error code: 401 - {'error': {'message': 'Incorrect API key provided:' ...}}`. ROOT CAUSE: Coolify `vexonhq-ocr-api` had duplicate `OPENAI_API_KEY` rows (`sk-proj-...` variants), so the container could read a stale/wrong/revoked project key even though TUM was looking at credit on another OpenAI project. FIX: TUM removed/replaced the duplicate rows so Coolify has exactly ONE `OPENAI_API_KEY`, generated a fresh key from the funded OpenAI project, restarted the backend, waited for `/health/deep` to settle, then tested a LINE slip successfully. RULES: (a) `OPENAI_API_KEY` must appear exactly once in Coolify env for `vexonhq-ocr-api`; never add a second row with the same name; edit/replace the existing row instead. (b) `401 Incorrect API key provided` points to key/project/auth mismatch; credit exhaustion is more likely `insufficient_quota`/billing/quota messaging. (c) Slip OCR path is `line_bot_routes._process_one_event` -> `_process_slip_from_line` -> `slip_routes._run_slip_vision` -> `llm.openai_chat` -> `OPENAI_API_KEY`, so it is OpenAI-backed, not Anthropic/Claude. (d) After changing env, restart/redeploy because `llm.py` reads provider keys at process import and the singleton OpenAI client caches the key. (e) Verify with `/health/deep` first (`openai_configured:true`, CPU settled) and then with a REAL LINE slip, because `/health/deep` only checks that a key is present; it does not call GPT Vision. Backup created before diagnosis: `C:\Users\rapee\Desktop\POS Export\backups\vexonhq-ai-provider-20260705-000045`.

**65. Bank statement LINE PAY / Grab rows are settlement movements, not new revenue; use dry-run before historical reclass.** (2026-07-06, pending review) TUM confirmed local evidence lives at `C:\Users\rapee\Desktop\PJ-MARA\VPS-VEXONHQ\Report POS` and that statement text `LINE PAY` can include non-delivery LINE Pay / QR / payment-gateway money. RULES: (a) do NOT classify `LINE PAY` / `ไลน์ เพย์` bank inflows as `rider_income_lineman`; classify them as `payment_gateway_payout` unless a separate settlement document proves delivery settlement. (b) `payment_gateway_payout` must be excluded from P&L anywhere `grab_payout` / `lineman_payout` are excluded, otherwise POS revenue is counted again when bank money lands. (c) Thai Grab text uses `แกร็บ` in KBank statements; it must map to `grab_payout`, overriding legacy `statement_rules` that mapped it to `rider_income_grab`. (d) KBank PDF page headers/footers can be extracted as wrapped continuation text; parser changes must keep `scripts/verify_statement_parse.py` in sync with `phase12_bank_statement_routes.py` and preserve the statement checksum. (e) Historical correction is a two-step workflow: first run `/bank-statement/reclass-dry-run` and review the candidate rows/month totals, then take a DB backup and ask TUM for explicit approval before any UPDATE. Never bulk-update old months from keyword alone.
**66. Historical statement reclass SQL must be reviewed-ID-only with transaction guards.** (2026-07-07) June 2026 reclass review found the main benefit is reconciliation/audit-trail accuracy, not P&L movement (the settlement sources are excluded from `v_daybook_pnl`, and legacy rider-income bank rows are dropped from `v_daybook`). Any production UPDATE must use a literal reviewed `id` set from the dry-run/export, not `description ILIKE` or current source-type keywords. Required guards: current `source_type`, `category_code`, `match_status`, `txn_date`, `credit`, and `branch_code` must still match the reviewed report; `match_status='manual'` rows are excluded unless TUM approves row-by-row; expected row count and credit sum must be asserted inside the transaction; backup must be captured before update; and rationale must be appended to `notes` for evidence-based overrides such as the June 7 Grab `345.79` row and June 30 Grab `551.31` row that originate from `pos_cash_deposit` and need Grab-export payout-date evidence.
**67. June 2026 statement reclass is committed; do not rerun it.** (2026-07-07) TUM explicitly confirmed `Confirm RUN JUNE RECLASS COMMIT`, then the June-only SQL was run from an in-memory COMMIT copy after a full ROLLBACK dry-run. The draft file remains rollback-safe on disk (`ROLLBACK;` active, `COMMIT;` commented). Persistent rollback backup table: `audit.bank_statement_reclass_backup_20260707_june` (`102` rows / `175,899.24`). Local evidence backup: `C:\Users\rapee\Desktop\PJ-MARA\VPS-VEXONHQ\Audit Reports\statement_reclass_commit_20260707_175258\before_bank_statement_entries_102_rows.json`. Verified current rows: `payment_gateway_payout` 76 / `161,719.11`, `grab_payout` 26 / `14,180.13`, manual overrides 2 (`345.79`, `551.31`). `v_daybook_pnl` June income/expense stayed unchanged (`225,924.63` / `201,929.67`). Raw `v_daybook` June income rose by `175,002.14`, not the full `175,899.24`, because the two `pos_cash_deposit` exception rows (`897.10`) were already raw income. Treat this raw-ledger movement as reconciliation visibility, not new sales or profit.
**68. May 2026 statement duplicate cleanup is committed; do not rerun it.** (2026-07-07) TUM explicitly confirmed `Confirm RUN MAY DUPLICATE CLEANUP COMMIT`, then the May duplicate-cleanup SQL was run from a COMMIT copy after ROLLBACK dry-run + Antigravity review. The draft file remains rollback-safe on disk (`docs/superpowers/plans/2026-07-07-may-statement-duplicate-cleanup-draft.sql`, `ROLLBACK;` active, `COMMIT;` commented). Persistent rollback backup tables: `audit.bank_statement_duplicate_cleanup_backup_202605_20260707` (`14` rows: 7 delete + 7 keep) and `audit.statement_duplicate_cleanup_slip_backup_202605_20260707` (`1` slip). Local evidence folder: `C:\Users\rapee\Desktop\PJ-MARA\VPS-VEXONHQ\Audit Reports\statement_duplicate_cleanup_may_commit_20260707_183229\` with before/after JSON and executed COMMIT copy. Verified after COMMIT: delete rows remaining `0`, keep rows `7`, target duplicate keys `0`, slip `5c788be1-50d5-4a9c-81d6-20c61efa81d9` now points to kept statement `89aa5df5-e7b0-4492-8d74-a40b3c8d62d7`; May `v_daybook_pnl` expense dropped from `275,926.22` to `275,226.22` (-`700.00`) because one musician-fee row had been double-counted. Income stayed `325,695.51`.
**69. May 2026 statement reclass is committed; do not rerun it.** (2026-07-07) TUM explicitly confirmed `Confirm RUN MAY RECLASS COMMIT`, then the May reclass SQL was run from a COMMIT copy after fresh dry-run, Claude/Antigravity review, and May duplicate cleanup. The draft remains rollback-safe on disk (`docs/superpowers/plans/2026-07-07-may-statement-reclass-draft.sql`, `ROLLBACK;` active, `COMMIT;` commented). Persistent rollback backup table: `audit.bank_statement_reclass_backup_20260707_may` (`110` rows / `234,703.16`). Local evidence folder: `C:\Users\rapee\Desktop\PJ-MARA\VPS-VEXONHQ\Audit Reports\statement_reclass_may_commit_20260707_223428\`. Verified after COMMIT: updated rows `110` / `234,703.16`; `payment_gateway_payout` `84` rows / `221,034.91`; `grab_payout` auto `25` rows / `13,154.93`; manual Grab exception `1` row / `513.32` (`a13569a8-7463-4874-b9a9-91d0372f8d67`, matched to May Grab CSV transfer-date aggregate 2026-05-30 = `513.32` / 4 rows). May `v_daybook_pnl` stayed unchanged after reclass (`income 325,695.51`, `expense 275,226.22`).
**70. April 2026 through November 2025 verified historical statement reclass batch is committed; do not rerun it.** (2026-07-07) TUM explicitly confirmed `Confirm RUN HISTORICAL RECLASS COMMIT` with scope `APRIL, MARCH, FEBRUARY, JANUARY, DECEMBER, NOVEMBER VERIFIED SUBSET ONLY` and `Do not touch the 11 pending early-November Grab rows.` COMMIT was run oldest-to-newest from copied SQL files only; the draft SQL files under `docs/superpowers/plans/` remain rollback-safe (`ROLLBACK;` active, `COMMIT;` commented). Local execution/log evidence folder: `C:\Users\rapee\Desktop\PJ-MARA\VPS-VEXONHQ\Audit Reports\historical_reclass_commit_20260707_231950\`. Persistent rollback backup tables: `audit.bank_statement_reclass_backup_20260707_april` (`91` / `207,763.87`), `audit.bank_statement_reclass_backup_20260707_march` (`105` / `212,939.86`), `audit.bank_statement_reclass_backup_20260707_february` (`98` / `179,513.36`), `audit.bank_statement_reclass_backup_20260707_january` (`102` / `197,126.35`), `audit.bank_statement_reclass_backup_20260707_december` (`102` / `181,553.15`), and `audit.bank_statement_reclass_backup_20260707_november_verified_subset` (`96` / `253,502.11`). Verified after COMMIT: April `grab_payout` `22` / `11,961.14` and `payment_gateway_payout` `69` / `195,802.73`; March Grab `27` / `15,432.36` (including manual parser-miss `588.18`) and gateway `78` / `197,507.50`; February Grab `25` / `10,485.88` and gateway `73` / `169,027.48`; January Grab `26` / `13,046.55` and gateway `76` / `184,079.80`; December Grab `27` / `10,813.80` and gateway `75` / `170,739.35`; November verified subset Grab `9` / `3,337.75` and gateway `87` / `250,164.36`. The excluded 11 early-November Grab rows remain untouched as `rider_income_grab` / `delivery_income` / `auto`, total `3,168.81`; do not reclass them until the missing `Transaction_Store_2025-11-01_to_2025-11-16...csv` is supplied or TUM explicitly approves bank-text-only evidence.
**71. The 11 early-November 2025 Grab manual-evidence rows are committed; do not rerun them.** (2026-07-09) TUM confirmed no additional Grab export exists beyond the local 2025-11-17..2025-11-30 file, then Antigravity reviewed `docs/superpowers/plans/2026-07-09-november-early-grab-manual-reclass-draft.sql` with no blocker and TUM explicitly confirmed `Confirm RUN NOVEMBER EARLY GRAB MANUAL COMMIT`. COMMIT was run via Supabase MCP because local direct `psql` to the Supabase pooler was timing out while `/health/deep` remained healthy. The draft remains rollback-safe on disk (`ROLLBACK;` active, `COMMIT;` commented). Persistent rollback backup table: `audit.bank_statement_reclass_backup_20260709_november_early_grab` (`11` rows / `3,168.81`). Verified after COMMIT: all 11 rows are `grab_payout` / `delivery_grab` / `manual` with `classified_by='codex_reviewed_november_early_grab_20260709'`; November `v_daybook_pnl` remained unchanged (`income 329,342.05`, `expense 351,348.59`); `/health/deep` healthy with CPU `0.0%`. This closes the April 2026 through November 2025 historical statement reclass batch; future sessions should not rerun June, May, April-November verified subset, or early-November manual scripts.
**72. Monthly-close risk marker V1 is read-only source detection; migration-first deploy.** (2026-07-09) New `monthly_close_routes.py` adds admin-only `POST /monthly-close/check` and `GET /monthly-close/risks`; the only table it writes is `public.monthly_close_risks`. It must never mutate `bank_statement_entries`, `pos_bills`, `pos_imports`, P&L views, or month-lock state. The table was applied live via Supabase MCP migration `monthly_close_risks_v1` before code deploy, with RLS enabled and zero policies. LINE is danger-only and throttled by `last_line_sent_at` (24h); failed LINE sends do not stamp the timestamp. R4 must keep `COALESCE(source_type, '') NOT IN (...)` so `NULL`/unclassified settlement rows are not missed. R3's danger mapping `public.pos_bills.payment_type_raw='K Plus shop'` = Grab depends on TUM's confirmed POS bill-detail mapping; if the POS meaning changes to generic KBank QR, downgrade that rule before enabling LINE for it. Add tests when changing risk SQL; current guard is `tests/test_monthly_close.py`.
**73. `bank_entry_bill_links` is authoritative for per-bill transfer membership; `matched_invoice_id` is a legacy first-link mirror only.** (2026-08-31) One bank transfer can settle several vendor bills, but `bank_statement_entries.matched_invoice_id` intentionally mirrors only the first linked bill for legacy readers. Any per-bill linked/unlinked flag, including `GET /bills/payment.bank_statement_entry_id`, must read `public.bank_entry_bill_links` by `vendor_bill_id`; otherwise later bills in a combined transfer appear unlinked. The unique `vendor_bill_id` index means one transfer per bill while allowing many bills per transfer. Keep the mirror refresh for compatibility, but do not treat it as complete membership. Regression guard: `test_list_bills_reads_link_table_not_the_legacy_mirror` in `tests/test_bill_payment_bank_link.py`. Shipped in `08cd34d`, deployed by `46fa408`, rollback tag `backup-pre-bill-link-flag-2026-08-31`.
**74. WHT/P.N.D. is decommissioned; category labels are not tax-event evidence.** (2026-09-01; pending deploy.) TUM confirmed the restaurant pays musician fees and THB 8,000 rent in full, VEXONHQ has no actual-withheld/remitted state, and no WHT was remitted through this workflow. Keep VAT, P&L, expense categories, history, and full paid amounts unchanged. `/tax/wht-summary`, `/tax/wht-export`, `/export/pnd3`, and `/export/pnd3-annual` are explicit HTTP 410 tombstones; never replace them with fake-zero files or silent 404s. ZIP contains Category Summary + Daybook only. Summary/readiness retain `decommissioned` compatibility objects; Audit schema v2 retains `wht:null`. Do not reintroduce `WHT_RULES`, rate math, payee tax-id prefills, or category-derived candidates. A future real WHT feature needs explicit actual-withheld and remitted-state data plus owner/accountant approval. Contract: `docs/superpowers/specs/2026-09-01-wht-decommission-backend-contract.md`.
*Last updated: 2026-07-06 (#64 OpenAI API key incident — LINE Slip OCR 401 `Incorrect API key provided` was Coolify duplicate/stale `OPENAI_API_KEY`, not Claude credit; keep exactly one `OPENAI_API_KEY` row, edit/replace instead of adding duplicate, restart/redeploy because `llm.py` reads env at import, verify `/health/deep` then a real LINE slip). Prev 2026-06-26 (#62 POS bill_detail import timeout was CONCURRENCY not slow SQL — overlapping re-uploads re-ran the whole heavy write (uq_pos_imports_hash is PARTIAL WHERE status='success' so dup only caught at final UPDATE) + an O(N) per-line-item SELECT loop ballooned a lone re-import to 626s; the 2-min statement_timeout on uq_pos_bills was the secondary tail; fixed in pos_import.py both endpoints with (A) dup short-circuit before any work, (B) session pg_try_advisory_lock per (branch,rtype) ~5s bounded + lock_timeout, (C) bulk INSERT…ON CONFLICT…RETURNING execute_values + order-independent batch-fallback removing the per-item loop; INVARIANT RETURNING cols = ON CONFLICT key not in update_cols so map key == parser _bill_key; 15/15 rolled-back real-DB integration test + EXPLAIN uq_pos_bills arbiter + Codex×2 + workflow red-team; NO data loss (623 June bills present); cleanup 72 noise rows→cancelled, 4 genuine failed preserved; commit 68858ad, rollback backup-pre-pos-import-fix-2026-06-26; batch_import_local.py per-item loop + multi-branch deferred). Prev 2026-06-21 (#61 `/dashboard/executive` sales card gains a DISPLAY-ONLY gross→net waterfall — `_SALES_WATERFALL_SQL` + pure `_sales_waterfall()` attach `{foodstory_gross, delivery_commission, other_adjustment, net_received}` to the `sales_mtd` card; FoodStory net_total is GROSS, `sales_net` is cash-basis net-of-Grab/Lineman-commission, the commission is ALREADY netted in `pos_sale` so it must NEVER be added to expense/profit (double-count); `net_received==summ.sales_net` always, `other_adjustment` reconciles pos_cashflow/floor-day months; executive route now does TWO fetchone()s so test_dashboard_pending_db mock updated; tests real Apr/May/Jun reconcile; commit 783f94c; rollback backup-pre-sales-waterfall-2026-06-21). Prev 2026-06-09 (#60 opt-in Sentry error tracking re-introduced (removed Session 68, re-added boot-safe) — init in main.py ONLY when SENTRY_DSN set, the whole import+init wrapped in try/except so a missing pkg or malformed DSN can never fail startup; [fastapi] extra auto-captures unhandled 500s with tracebacks (no middleware/webhook/debug-endpoint); release=APP_VERSION single 3.7.0 constant, env default production, tracing off; NO Telegram (Discord-only); dep in requirements.txt which the Dockerfile installs; commit ff66d37 deployed but DISABLED until SENTRY_DSN set in Coolify; rollback backup-pre-sentry-2026-06-09). Prev 2026-06-09 (#59 invoice upload idempotency now keys on a FILE-LEVEL byte SHA-256 (`attachments.file_sha256`, migration 2026_06_09) checked PRE-OCR — replaces the fragile non-deterministic OCR-content comparison that triplicated a re-uploaded Makro bill's items; dup hit returns existing bill + already_uploaded:true + skips OCR/cost; column nullable+partial-index+NON-unique because multi-page shares one hash; no backfill; fail-open; tests/test_ocr_file_hash.py; builds on #58). Prev 2026-06-08 (#58 multi-page invoice OCR parallelized — GPT-Vision stage runs concurrently cap 3, persist/merge stays sequential in page order; fixes the Cloudflare 524 false "upload failed" on multi-page Makro PDFs without splitting one bill into duplicates; backend-only, single-image path unchanged; tests/test_ocr_parallel_pages.py guards order+serial-persist+cap; file-hash idempotency + async job queue deferred per lean-bar). Prev 2026-06-03 (#57 SEC-3 complete — frontend repos only, backend Bearer token unchanged; GP% costs entered for 25 Wongnai recipes via recipe_ingredients; Supabase downgraded to Free ~270 MB confirmed; all systems 200). Prev 2026-06-03 (#56 Wongnai/FoodStory menu sync — recipes has NO FoodStory menu_id so sync matches by code-prefix then normalized Thai-name; tooling committed b7ae41e: scripts/wongnai_analyze.py read-only + scripts/wongnai_apply.py apply; Phase 1 125/143 matched; Phase 2 APPLIED + verified — recipes backed up first (141 rows), 16 selling_price → in-store, 141 images imported to Supabase menu-images bucket + image_url set, 16 new items inserted → 157 total; images serve 200 image/webp through marastation-web Next optimizer; RE-SYNC = fresh wongnai_snapshot THEN wongnai_apply.py — the in-script 403 image-key refresh is a placeholder/unreliable; hardcoded x-api-key is the PUBLIC Wongnai client key, not a secret; 16 recipes-only promos/fees left untouched). Prev 2026-06-03 (#55 PNL-3 ภ.ง.ด.3 WHT gross-vs-net RESOLVED, verdict GROSS — Antigravity's net-assumption gross-up REVERTED, NO code change; settled by live-data round-number test on v_daybook_pnl (115/120 musician_fee rows round-to-100); cross-agent adversarial-review catch — wrong tax formula on a govt filing caught before shipping; minor deferred: rename tax_routes.py net_before_wht display field). Prev 2026-06-03 evening (#54 A+ remediation round 4 → A/A+: OPS-13 REAL DB-outage fix — transaction-mode-safe + connect-retry + Coolify DATABASE_URL→:6543; SEC-1b /ai/exec IP allow-list; PNL-4 ภ.ง.ด.3 exact-name tax-id prefill; AI-6 cashflow AI decision log; OPS-11 watchdog self-alert fix; duplicate-operationId cleanup + Haiku-auto-diagnose-was-wrong defense note; 211 tests passed). Prev 2026-06-03 (#53 A+ remediation round 3 → A/A+: OCR-1 confirm-gating 422 CONFIRM_BLOCKED, OCR-2 cross-vendor invoice-merge guard, OPS-11 active cron stale-job Discord watchdog, OPS-4 pg_dump on :5432, OPS-10 18 money-path tests + verify.ps1 [1b]; FE-6 /pos/compare safeFetch; re-graded OCR-1/2 to HIGH; 204 tests passed). Prev 2026-06-03 (#52 A+ remediation round 2 → ~A: SEC-4 Discord anti-replay, OPS-6 disk in /health/deep, OPS-8 snapshot floor, AI-5 forecast no-guess, PNL-2 budget cash-basis, OPS-9 rotate verify-before-delete, dup-index + 3 dead-table drops, SEC-2 web.* RLS; OPS-2 correction kept; sales_import_raw kept). Prev 2026-06-03 (#51 2026-06-02 audit B+ + A+ remediation round 1 → A-: /ai/exec SEC-1, OPS-1 DR backup, AI-2, OPS-3, OCR-3, AI-8, PNL-1 budget view). Prev 2026-06-01 (#50 OCR accuracy from production F1; #49 slip anti-tamper F11; #48 statement-import checksum AUD-DATA-01; #47 audit batch 2; #46 admin-gate; #45 secdef-views revoke).*
